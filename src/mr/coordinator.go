	...
	if c.isMapStage() {
		for i, task := range c.MapTasks {
			...
			if task.State == 0 {
				...
				response.MapNum = len(c.MapTasks)
				return nil
			}
		}
...
func (c *Coordinator) ReportTaskDone(args *ReportTaskArgs, reply *ReportTaskTaskReply) error {
	c.mu.Lock()
	defer c.mu.Unlock()
	if args.JobType == TaskTypeMap {
		c.MapTasks[args.JobNum].State = 2
	} else if args.JobType == TaskTypeReduce {
		c.ReduceTasks[args.JobNum].State = 2
	}
	reply.Ack = true
	return nil
}
...
func (c *Coordinator) Done() bool {
	...
	defer c.mu.Unlock()
	ret := false
	if c.isMapStage() && c.allTasksDone(c.MapTasks) {
		c.Stage = "reduce"
	} else if c.isReduceStage() && c.allTasksDone(c.ReduceTasks) {
		c.Stage = "done"
		ret = true
	}
	return ret
